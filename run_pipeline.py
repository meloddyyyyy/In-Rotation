...
# 2. Pipeline order
# ==================================================
# Core production pipeline must finish before
# experimental candidate expansion. Candidate
# scripts write only to candidate_* files and
# must not overwrite auto_google_*.csv.
...
def main():
    ...
    for offset, script_name in enumerate(
        CANDIDATE_PIPELINE
    ):
        ...
        try:
            ...
            warning = (
                f"{script_name} failed after the core pipeline "
                f"completed: {error}"
            )
            ...
            print(
                "Core production outputs were left unchanged."
            )
